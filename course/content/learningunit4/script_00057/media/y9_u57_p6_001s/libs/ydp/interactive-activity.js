(function (wnd) {
	...
	function checkSingleWords() {
		...
		for (var i = 0; i < words.length; i++) {
			...
			if (lettersTodo === lettersDone) {
				...
				a1 += [words[i]];
				const a2 = ["cycling", "swimming", "hiking", "shopping", "riding", "sailing", "sweating", "drinking", "exercising", "dancing"];
				var equal = a2.filter(d => a1.includes(d));
				var difference = a2.filter(d => !a1.includes(d));

				equals = '<del>' + equal.join('<br/>') + '</del><br/>';

				setTimeout(function () {
					let div_txts = document.querySelector("#txt-tips");
					div_txts.innerHTML = "";

					for (let i = 0; i < equals.length; i++) {
						div_txts.innerHTML = equals + difference.join('<br/>');
					}
				}, 50);
	...
	function updateTimeProgress() {
		...
		var progW = (graphVersion === 'lower') ? 140 : 142;
	...
	var reset = ["cycling<br/>", "swimming<br/>", "hiking<br/>", "shopping<br/>", "riding<br/>", "sailing<br/>", "sweating<br/>", "drinking<br/>", "exercising<br/>", "dancing<br/>"];
	...
	function resetGame() {
		...
		document.querySelector("#txt-tips").innerHTML = "";
		document.querySelector("#txt-tips").innerHTML = reset.join("");
		clearInterval(timerID);
		if (currentSound) {
			innerDisrupt = true;
			currentSound.stop();
		...
		for (var i = 0; i < maxY; i++) {
			...
			for (var j = 0; j < maxX; j++) {
				...
				if (cellsaved.ok) {
					...
					console.log('word ' + cellsaved.word);
				} else {
					cell.removeAttr('data-ok').removeAttr('data-allwordok').removeAttr('data-word');
				}
